class KnightsTour:
    #các bước di chuyển của mã
    moveX = [2, 1, -1, -2, -2, -1, 1, 2]
    moveY = [1, 2, 2, 1, -1, -2, -2, -1]


    def __init__(self, n):
        #hàm khởi tạo (n là kthuoc bàn cờ)
        self.n = n
        #lấp đầy bảng = -1
        self.board = [[-1 for i in range(n)] for j in range(n)]
        self.stack = [] #tạo list rỗng chứa các bước hợp lệ

    def s(self, x, y, stepsTaken):
        if stepsTaken == self.n * self.n:#chạy đủ hết bàn cỡ thì đọc ra
            # ptu 1 xong in ra bảng
            for r in self.board :
                print(r)
            return True

        possible_moves = []#các TH mã có thể đi
